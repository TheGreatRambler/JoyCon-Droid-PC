      ...
      if (something_new) {
        ...
        for (int i = 0; i < num_hats; ++i) {
          ...
          printw(
              ...
              ((hats[i] & SDL_HAT_UP) &&
               !(hats[i] & (SDL_HAT_LEFT | SDL_HAT_RIGHT)))
                  ? 'O'
                  : ' ',
...
void test_gamecontroller_state(SDL_GameController *gamepad) {
  ...
  while (!quit && SDL_WaitEvent(&event)) {
    ...
    for (int btn = 0; btn < SDL_CONTROLLER_BUTTON_MAX; ++btn) {
      printf("%s:%d ", SDL_GameControllerGetStringForButton(btn),
             SDL_GameControllerGetButton(gamepad, btn));
    }

    for (int axis = 0; axis < SDL_CONTROLLER_AXIS_MAX; ++axis) {
      printf("%s:%6d ", SDL_GameControllerGetStringForAxis(axis),
  ...
  if (SDL_Init(SDL_INIT_TIMER | SDL_INIT_VIDEO | SDL_INIT_JOYSTICK |
    ...
    if (argc == 2 &&
      ...
      if (!str2int(argv[2], &idx)) {
        fprintf(stderr,
                "Error: JOYSTICKNUM argument must be a number, but was '%s'\n",
                argv[2]);
        exit(1);
      } else {
